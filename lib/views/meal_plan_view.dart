                ...
                return ExpansionTile(
                  ...
                  children: mealPlan.mealsPerWeek.map((meal) {
                    ...
                    return ListTile(
                      ...
                      subtitle: Text(
                          '${meal.price} EUR - ${meal.type.toString().split('.').last}'),
                      trailing: UserSession.isAdmin()
                          ? IconButton(
                              icon: Icon(Icons.edit),
                              onPressed: () {
                                _showManageMealDialog(
                                    context, meal, mealPlan, index);
                              },
                            )
                          : null,
                    );
...
//Dialog/Popup zum Bearbeiten eines Essensplans
  void _showManageMealPlanDialog(BuildContext context,
      {MealPlan? mealPlan, int? index}) {
    ...
    showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          title: Text(
              isNew ? 'Neuen Essensplan hinzufügen' : 'Essensplan bearbeiten'),
          ...
          actions: [
            ...
            TextButton(
              onPressed: () {
                if (weekNumberController.text.isNotEmpty &&
                  ...
                  if (isNew) {
                    ...
                    widget.controller.updateMealPlan(
                        index!,
                        MealPlan(
                          weekNumber: weekNumber,
                          mealsPerWeek: selectedMeals,
...
//Dialog/Popup zum Bearbeiten eines Essens
  void _showManageMealDialog(
      BuildContext context, Food meal, MealPlan mealPlan, int index) {
    final TextEditingController nameController =
        TextEditingController(text: meal.name);
    final TextEditingController priceController =
        TextEditingController(text: meal.price.toString());
            ...
            TextButton(
              onPressed: () {
                if (nameController.text.isNotEmpty &&
                  ...
                  widget.controller.updateMealPlan(
                      index, mealPlan); // Aktualisiere den MealPlan
                  Navigator.of(context).pop();
                  setState(() {}); // Aktualisiere die Ansicht
                }
              },
